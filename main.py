...
class AdminPage:
    ...
    def login(self):
        ...
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='wenbin2004.04.02') # 打开数据库连接
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        self.age = []
        # 打开数据库连接
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='wenbin2004.04.02')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
...
class Changekey:
    ...
    def login(self):
        ...
        stu_pass = None
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='wenbin2004.04.02')  # 打开数据库连接   已经修正删除多余空格
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
